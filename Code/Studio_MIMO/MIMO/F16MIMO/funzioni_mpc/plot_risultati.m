...
function plot_risultati(t_sim, storia_x, storia_u, U_min, U_max)
    ...
    yline(U_max(2), 'k--'); yline(U_min(2), 'k--'); title(' Elevatore [rad]'); grid on; ylim([U_min(2)-5, U_max(2)+5]);
    subplot(3,1,3); stairs(0:t_sim-1, storia_u(3,:), '-g', 'LineWidth', 1.5); hold on;
    yline(U_max(3), 'k--'); yline(U_min(3), 'k--'); title(' Flap [rad]'); grid on; ylim([U_min(3)-5, U_max(3)+5]);
end
