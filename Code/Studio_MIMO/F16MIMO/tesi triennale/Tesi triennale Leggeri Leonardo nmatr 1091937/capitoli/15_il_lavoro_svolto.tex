...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{ImageSender} e \texttt{ImageSubscriber}: scambio di foto su rete DDS.
...
Ed è proprio all'interno di questo metodo che sono stati iniettati i nostri kernel marker: ciò ci permette di notare con assoluta precisione nel file di trace quando effettivamente il Listener riceve e notifica un messaggio sulla rete DDS. Il \texttt{Listener} a sua volta incapsula la creazione e la registrazione sicura del \texttt{DomainParticipant}, \texttt{Subscriber} e \texttt{DataReader}.
...
\begin{lstlisting}[language=bash, caption={\texttt{marker.sh} -- Chiamata per primo e secondo processo}]
	...
	# 2. Se abbiamo un secondo eseguibile, cerchiamo anche lui e i suoi thread
...
è che qui non viene passato nessun eseguibile alla fine del comando: la
...
file manca. Anche qui KernelShark viene aperto in background con la stessa
...
\begin{lstlisting}[language=C++, caption={\texttt{thread\_analysis.cpp} -- Ingresso del main e mappe di stato}]
	...
	La quarta fase elabora la mappa \texttt{tasks} ora completa e, per ogni thread,
