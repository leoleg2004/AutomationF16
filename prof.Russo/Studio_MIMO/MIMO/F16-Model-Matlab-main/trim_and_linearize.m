...
% Esegui Trim e linearizzazione e mostra le sottomatrici longitudinali
[A_long,B_long,C_long,D_long,info] = extract_and_print_ABCD(Param,'F16');
% ora A_long, B_long, C_long, D_long e info sono disponibili nel workspace base
...
function [A,B,C,D,info] = extract_and_print_ABCD(Param, modelName)
if nargin<2 || isempty(modelName), modelName = 'F16'; end

% assegna parametri nel workspace base se forniti
...
% esegui TrimF16 (deve essere sul path)
TrimF16; % crea 'op' nel workspace

% linearizza
linsys = linearize(modelName, op);
...
iu = [1,2,5,6,8];%spinta,equilibratore,flap,raffiche(assex),raffiche(z),nel simualtore rispetto a Lewis aggiungo anche la dinamica dl 
% vento per avere una matrice B più accurata 
iy = [7,8,13,15,17,19];%vt,alfa(prndo questi due all'uscita perchè il simulink prende integrando 
% accellerazioni U,W e qua prendo vt,e alfa angolo di attacco per ritonare come risultato all'uscita 
%come vuole Lewis
